...
def main(args):
    ...
    assert len(exp_names) >= 1, exp_names
    ...
    jtt_color_list = ["mediumpurple", "darkviolet", "purple"]
    mapd_color_list = ["lightcoral", "orangered", "firebrick"]
    
    run_idx_list = {}
    for exp_name in exp_names:
        print("Loading exp:", exp_name)
        assert "_seed_" in exp_name
        exp_name_base, exp_seed = exp_name.split("_seed_")
        if exp_name_base not in run_idx_list:
            run_idx_list[exp_name_base] = 0
        run_idx_list[exp_name_base] += 1
        exp_seed = int(exp_seed)
        print(f"Experiment name base: {exp_name_base} / Selected seed: {exp_seed} / Run Idx: {run_idx_list[exp_name_base]}")
        ...
        csv_file = os.path.join(data_dir, f"output_train_epoch_{final_epoch}.csv")
        print("Reading main output file:", csv_file)
        train_df = pd.read_csv(csv_file)
        train_df = train_df.sort_values(f"indices_None_epoch_{final_epoch}_val")
        train_df["wrong_1_times_jtt"] = (1.0 * (train_df[f"y_pred_None_epoch_{final_epoch}_val"] != train_df[f"y_true_None_epoch_{final_epoch}_val"])).apply(np.int64)
        ...
        if dataset == "MultiNLI":
            ...
            train_probs_df["probs_0_jtt"] = probs[:,0]
            train_probs_df["probs_1_jtt"] = probs[:,1]
            ...
            if dataset == 'CelebA':
                ...
                train_probs_df["confidence_jtt"] = train_probs_df["Blond_Hair"] * train_probs_df["probs_1_jtt"] + (1 - train_probs_df["Blond_Hair"]) * train_probs_df["probs_0_jtt"]
            elif dataset == 'CUB':
                train_probs_df["confidence"] = train_probs_df["y"] * train_probs_df["probs_1"] + (1 - train_probs_df["y"]) * train_probs_df["probs_0"]
                train_probs_df["confidence_jtt"] = train_probs_df["y"] * train_probs_df["probs_1_jtt"] + (1 - train_probs_df["y"]) * train_probs_df["probs_0_jtt"]
            elif dataset == 'jigsaw':
                ...
                train_probs_df["confidence_jtt"] = (train_probs_df["toxicity"] >= 0.5) * train_probs_df["probs_1_jtt"] + (train_probs_df["toxicity"] < 0.5)  * train_probs_df["probs_0_jtt"]
        ...
        # Plot the ROC curve
        out = RocCurveDisplay.from_predictions(merged_csv["spurious"], train_probs_df["probs_1_jtt"], ax=ax, name=f"JTT (Run # {run_idx_list[exp_name_base]})")
        # out.line_.set_linewidth(3.)
        out.line_.set_color(jtt_color_list[run_idx_list[exp_name_base]-1])
        # out.line_.alpha = 0.8
        out2 = RocCurveDisplay.from_predictions(merged_csv["spurious"], train_probs_df["probs_1"], ax=ax, name=f"MAP-D (Run # {run_idx_list[exp_name_base]})")
        # out2.line_.set_linewidth(3.)
        out2.line_.set_color(mapd_color_list[run_idx_list[exp_name_base]-1])
        # out.line_.alpha = 0.8
    ...
    plt.tight_layout()
    plt.savefig(f"roc_curve_{args.dataset.lower()}_comb.png", dpi=300, bbox_inches="tight")
